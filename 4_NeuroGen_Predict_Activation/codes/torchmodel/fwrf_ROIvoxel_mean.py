...
parser = argparse.ArgumentParser(prog='encoding model', 
	...
	usage='python fwrf_ROIvoxel_mean.py --subj i')

parser.add_argument('--subj', type=int)
parser.add_argument('--gpu', type=int)
args = parser.parse_args()

nsd_root = "E:/NeuroGen/Image_synthesis/NeuroGen-main/"
stim_root = nsd_root + "stimuli/nsd/"
...
_fmaps_fn, lmask, fmask, tuning_masks = filter_dnn_feature_maps(image_data, _fmaps_fn, batch_size=50, fmap_max=512)
print('filter done')
_x = torch.tensor(image_data[:100]).to(device) # the input variable.
_fmaps = _fmaps_fn(_x)
for k,_fm in enumerate(_fmaps):
